...
data_recom=np.loadtxt('./CalLines/recommended_TC4use_2.txt', dtype='unicode')

def lineDB(line_num):
  if line_num < len(data_recom):
    lineDBname=data_recom[line_num].split('_')[0] + '_' + data_recom[line_num].split('_')[1][:3]
...
def linemodel(species,xval,params,consts=[]):
    ...
    #model_y = 0 
    consts = 1
    ene,lg,pr,con = energy,lgamma,prob,consts
...
for species_use in range(len(data_recom)):
  try :
      ...
      energyrange_min=energymin-30
      energyrange_max=energymax+30
      pixel='all'
      ...
      plt.grid(linestyle='dotted',alpha=0.5)    
      plt.savefig("../figures/fit2_line%s_pix%s.png" % (species_use,pixel))
      plt.clf()
      plt.close()
      #plt.show()
      f = open('linefitting_result_all.txt','a')
      f.write('%f %f %f %E %E \n' % (energy, fwhm, fwhme, gain-1, gaine))
      f.close()
  except :
