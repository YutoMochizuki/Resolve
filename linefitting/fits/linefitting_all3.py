...
def lineDB1():
    lineDBname='V_Ka'
    filename="./CalLines/V_Ka6lor_Chant.dat"
...
def lineDB2():
    lineDBname='Ti_Kb'
    filename="./CalLines/Ti_Kb4lor_Chant.dat"
    data=np.loadtxt(filename, comments='#')
    energyDB=data[:,0]
    lgammaDB=data[:,1]
    ampDB=data[:,2]
...
try :
      ...
      plt.grid(linestyle='dotted',alpha=0.5)    
      plt.savefig("../figures/fit3_line%s_pix%s.png" % (species_use,pixel))
      plt.clf()
      plt.close()
      #plt.show()
